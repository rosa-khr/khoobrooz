...
## استاندارد تست و داکیومنت برای سرویس‌ها

هر بخش Backend که با منبع بیرونی کار می‌کند باید تست و داکیومنت داشته باشد.

موارد مهم در خوبروز:

- sync نرخ ارز
- crawler یا reader برای منابع بازار
- Jobهای زمان‌بندی شده
- API clientهای بیرونی
- import داده static مثل countries

قانون‌ها:

- سرویس بیرونی بدون `Feature Test` یا `Unit Test` پذیرفته نمی‌شود.
- تست‌ها باید از `Mock/Fake Response` استفاده کنند و به اینترنت واقعی وابسته نباشند.
- برای خطاهای سرویس بیرونی هم تست نوشته می‌شود.
- برای هر سرویس، داکیومنت فارسی نوشته می‌شود و اصطلاحات فنی انگلیسی می‌مانند.
- اگر sync داده انجام شود، باید log و وضعیت اجرای آن قابل بررسی باشد.
